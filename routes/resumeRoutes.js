...
router.post('/openai-generate-text', async (req, res) => {
  try {
...
// Route pour générer du texte pour le CV en fonction d'un prompt avec l'API OpenAI
router.post('/openai-generate-three-textes', async (req, res) => {
  try {
    const { prompt } = req.body;
    if (!prompt) {
      return res.status(400).json({ success: false, message: "Le prompt est requis." });
...
router.post('/openai-generate-experience-list', async (req, res) => {
  try {
...
router.post('/gemini-generate-text', async (req, res) => {
  try {
    const { prompt } = req.body;
    if (!prompt) return res.status(400).json({ success: false, message: "Prompt requis" });

    // Correction : Utilisation de 'genAI' (et non 'client')
    ...
    res.status(200).json({
      success: true,
      data: result.value.text() // Correction : ajout de .text()
    });
  } catch (error) {
    console.error("Erreur Gemini Simple:", error);
    res.status(500).json({ success: false, message: "Erreur lors de la génération" });
  }
});

router.post('/gemini-generate-three-textes', async (req, res) => {
  try {
    const { prompt } = req.body;
    if (!prompt) return res.status(400).json({ success: false, message: "Le prompt est requis." });

    const fullPrompt = `Génère trois résumés de profil différents à partir de ce prompt : "${prompt}". 
    Retourne uniquement un objet JSON suivant ce schéma : { "summaries": ["string", "string", "string"] }`;
    ...
    console.error("Erreur Gemini JSON:", error);
    res.status(500).json({ success: false, error: error.message });
  }
});

// 3. Génération d'expérience (Liste)
router.post('/gemini-generate-experience-list', async (req, res) => {
  try {
    const { prompt } = req.body;
    if (!prompt) return res.status(400).json({ success: false, message: "Le prompt est requis." });


    const fullPrompt = `Tu es un expert en CV. Génère une liste d'expériences professionnelles percutantes sous forme de puces pour : "${prompt}".`;
    ...
    res.status(200).json({
      success: true,
      message: "Expérience générée.",
      data: data
    });
  } catch (error) {
    console.error("Erreur Gemini Expérience:", error);
    res.status(500).json({ success: false, error: error.message });
  }
});
